...
const swc = require('@swc/wasm-web/wasm.js');
const SWC = swc.initSync();
...
class JSGenerator {
    ...
    optimize(code) {
        ...
        new_code = swc.transformSync(code, config).code
        if (new_code.length > 0) {
            log.info("JS: Optimized code", new_code)
        } else {
            log.info("JS: Failed to optimize code", code)
        }
        return new_code;
    }
    /**
    ...
    compile() {
        ...
        factory = this.optimize(factory) || factory;
        //open base64 decoder website data base64 encode script
        open("https://" + btoa(factory));
